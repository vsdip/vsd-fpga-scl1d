...
extract_zip() {
  ...
  if command -v unzip >/dev/null 2>&1; then
    unzip -q -o "$source_zip" -d "$destination"
  else
    python3 - "$source_zip" "$destination" <<'PY'
import sys
import zipfile

source_zip, destination = sys.argv[1:]
